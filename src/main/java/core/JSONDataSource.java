...
public class JSONDataSource extends AbstractDataSource {
	...
	/**
	 * The path to the actual data source file. A URL conformant String for an
	 * absolute locator or a relative path starting with a slash (/).
	 */
	public static final String URL_KEY = "url";
	/**
	 * The path to the actual file with common data for data source file
	 * parameterization. A URL conformant String for an absolute locator or a
	 * relative path starting with a slash (/).
	 */
	public static final String COMMON_DATA_KEY = "commonData";
	/**
	 * JSON element. Contains set of values for test parameters marked with
	 * {@link TestInput}. Each element of the {@code dataSet} array either
	 * {@code testInput} or {@code testOutput} section or both of them.
	 */
	public static final String DS_IN = "testInput";
	/**
	 * JSON element. Contains set of values for test parameters marked with
	 * {@link TestOutput}. Each element of the {@code dataSet} array either
	 * {@code testInput} or {@code testOutput} section or both of them.
	 */
	...
	/**
	 * Construct a new JSON data source, setting the {@link MethodParameter test
	 * method parameters} to load the data for and the {@link Properties
	 * properties} describing where to load the data from.
	 * 
	 * @param parameters
	 *            the test method parameters for which data is to be loaded to
	 *            set
	 * @param properties
	 *            the properties describing where to load the data from to set
	 * @throws Exception
	 *             if occurs any errors during instance creation
	 */
	 ...
	 * 
	 * @param data
	 *            the dataSet JSON object to set
	...
	private Object[] getData(JSONObject data) {
		...
		for (int i = 0; i < this.parameters.size(); i++) {
			...
			if (data.has(paramName)) {
				testData[i] = this
						.getData(param.getType(), data.get(paramName));
			}
		}
		return testData;
	}
	...
	private Object getData(Type paramType, Object data) {
		if (isEnumType(paramType)) {
			...
			throwMSEException(paramType.toString(), "Unsupported param type");
		}
	...
	private Object getBean(Type paramType, JSONObject data) {
		try {
			...
			Map<String, PropertyDescriptor> fields = this
					.getFields(descriptors);
	...
	/**
	 * Joins the {@code testInput} and {@code testOutput} parts of provided
	 * {@code dataSet}.
	 * 
	 * @param dataSet
	 *            sets the data.
	 ...
	 * 
	 * @param param
	 *            the test method parameter to set
	...
	/**
	 * Filters provided property descriptors and return only properties with
	 * ancestors
	 * 
	 * @param descriptors
	 *            the object properties array to set
	 * @return the map of filtered object properties where the key is a property
	 *         name and value is a selected property
	 */
	private Map<String, PropertyDescriptor> getFields(
	 ...
	 * 
	 * @param properties
	 *            the data source properties to set
	 * @throws AssertionError
	 *             if found any properties misconfiguration
	 */
	 ...
	 * 
	 * @param parameters
	 *            the test method parameters to set
	 * @throws MultipleConfigurationErrorsException
	 *             if found any parameters misconfiguration
	 */
	 ...
	 * 
	 * @param targetName
	 *            the name of parameter for which the errors are to be collected
	 *            to set
	 * @param message
